...
export default function PackTear({
  ...
  const getInterpolatedTrail = useCallback((trail: TrailPoint[], now: number) => {
    ...
    for (let i = 0; i < trail.length - 1; i++) {
      ...
      for (let t = 1; t <= steps; t++) {
        ...
        const tt = s * s;
        const ttt = tt * s;
        const x = 0.5 * ((2 * p1.x) + (-p0.x + p2.x) * s + (2 * p0.x - 5 * p1.x + 4 * p2.x - p3.x) * tt + (-p0.x + 3 * p1.x - 3 * p2.x + p3.x) * ttt);
        const y = 0.5 * ((2 * p1.y) + (-p0.y + p2.y) * s + (2 * p0.y - 5 * p1.y + 4 * p2.y - p3.y) * tt + (-p0.y + 3 * p1.y - 3 * p2.y + p3.y) * ttt);
        result.push({ x, y, time: p1.time + (p2.time - p1.time) * s, velocity: p1.velocity + (p2.velocity - p1.velocity) * s });
      }
    }
  ...
  const renderFrame = useCallback((ctx: CanvasRenderingContext2D, deltaTime: number) => {
    ...
    if (smoothTrail.length > 1) {
      ...
      for (let layer = 0; layer < 3; layer++) {
        ...
        const globalAlpha = age > fadeStart ? Math.max(0, 1 - (age - fadeStart) / (TRAIL_DURATION - fadeStart)) : 1;
  ...
  useEffect(() => {
    ...
    const animate = () => {
      const now = performance.now();
      const deltaTime = Math.min(now - st.lastTime, 50);
      st.lastTime = now;
      renderFrame(ctx, deltaTime);
      st.animationId = requestAnimationFrame(animate);
    };
    st.animationId = requestAnimationFrame(animate);
    return () => { cancelAnimationFrame(st.animationId); };
  }, [renderFrame]);
  ...
  useEffect(() => {
    ...
    st.guideAnimationId = requestAnimationFrame(animateGuide);
    return () => { cancelAnimationFrame(st.guideAnimationId); };
  }, [renderGuide]);
  ...
  const addBinsCrossed = (x0: number, x1: number) => {
    ...
    let a = Math.max(0, Math.floor(Math.min(x0, x1) / binWidth));
    let b = Math.min(BIN_COUNT - 1, Math.floor(Math.max(x0, x1) / binWidth));
  ...
  const move = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    for (const pev of coalesced) {
      ...
      st.trail.push({ x, y, time: now, velocity: st.velocity });
      const particleCount = Math.floor(distance * PARTICLE_SPAWN_RATE);
      if (particleCount > 0) spawnParticles(x, y, particleCount, st.velocity);
      st.cursorPos = { x, y };
