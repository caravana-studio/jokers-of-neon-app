...
export const CreatingGameProgressBar = ({
  ...
  const [currentHeading, setCurrentHeading] = useState(
    headingStages[0]?.text || "Creating game"
  );
  ...
  const applyNaturalEasing = (progress: number): number => {
    ...
    if (progress < 0.2) {
      ...
      return Math.min(
        1,
        Math.max(0, 0.3 + (progress - 0.2) * 0.8 + jitter * (1 - progress))
      );
    } else {
      // Speed up slightly at the end
      return Math.min(
        0.95,
        Math.max(0, 0.78 + (progress - 0.8) * 0.85 + jitter * (1 - progress))
      );
    }
  };
  ...
  useEffect(() => {
    if (isOpen && headingStages.length > 0) {
      ...
      const stagesInReverse = [...headingStages].sort(
        (a, b) => b.showAt - a.showAt
      );
      const currentStage = stagesInReverse.find(
        (stage) => elapsedTime >= stage.showAt
      );
